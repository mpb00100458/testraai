...
import { generateExcelReport, generateJsonReport, generateMarkdownReport, ensureVideosDir, ensureScreenshotsDir } from './exportUtils.js';
...
class AccessibilityMCPServer {
            ...
            try {
                switch (name) {
                    case "scan_url_accessibility":
                        return await this.scanUrlAccessibility(args);
                    case "get_wcag_guidance":
                        return await this.getWcagGuidance(args);
                    case "check_element_accessibility":
                        return await this.checkElementAccessibility(args);
                    case "generate_accessibility_report":
                        return await this.generateAccessibilityReport(args);
                    default:
    ...
    async scanUrlAccessibility(args) {
        const { url, wcagLevel = "AA", outputFormat = "text", saveToFile = outputFormat !== "text", headless = true, recordVideo = false, captureScreenshots = false } = args;
        ...
        try {
            ...
            const allViolations = violations.map((v) => ({
                id: v.id,
                impact: v.impact,
...
`;
    ...
    async checkElementAccessibility(args) {
...
`;
    ...
    async generateAccessibilityReport(args) {
