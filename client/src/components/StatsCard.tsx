...
export function StatsCard({ title, value, icon: Icon, trend, description }: StatsCardProps) {
  return (
    <Card className="gradient-border overflow-visible">
      ...
      <CardContent>
        <div className="text-3xl font-bold gradient-text" data-testid={`stat-${title.toLowerCase().replace(/\s+/g, '-')}`}>{value}</div>
        {trend && (
          <div className="flex items-center gap-1 mt-1">
            ...
            <span className={`text-xs ${trend.isPositive ? 'text-success' : 'text-critical'}`}>
              {trend.value}%
            </span>
          </div>
        )}
